...
class Pruebas{
  ...
  protected $Tipo;
    ...
    $db="mmweb";
    $con=mysqli_connect($server,$user,$pass,$db) or die ("Error en la conexión".mysql_error());
		$sql="DELETE FROM `cliente` WHERE idCliente=$this->Id_Cliente";
		$con->query($sql) ? header("location: clientes.php?res=Eliminado") : header("location: clientes.php?res=error");
	}
    ...
    $db="mmweb";
    $con=mysqli_connect($server,$user,$pass,$db) or die ("Error en la conexión".mysql_error());
		$sql = "SELECT * FROM `cliente` WHERE `nombre` LIKE '$this->Nombre%';";
		$result = $con->query($sql);
		$con->query($sql) ? header("location:clientes.php?Nombre=$this->Nombre") : header("location: clientes.php?res=error");
	}
